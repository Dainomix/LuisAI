...
var ltaApiKey = "HkRe6TjESjWH0OC7DkTZCw==";
...
.matches('Conversation.Greeting', (session, args) => {
    ...
    // Input E.g bus 131 at 50089
    // Input E,g bus 69 at 75009
.matches('BusTiming.NextBus', (session, args) => {
    ...
    if(busStopNum){
        ...
        //Default fallback bus stop number
        busStopNum = "75239";
    }
    
    // E.g. BusStopID=50089&ServiceNo=131
    // E.g. BusStopID=75009&ServiceNo=69
    request({headers: {'AccountKey': ltaApiKey}, uri:busUrl+"BusStopID="+busStopNum+"&ServiceNo="+busNum}, function(error, response, body){
        if(error){ 
        ...
        if(service != null){
            ...
            session.send("Bus "+busNum+" does not call at bus stop "+busStopNum+". Please check your details and try again.");
        }
